...
CREATE TABLE PASO_A_PASO.Usuario (user_id int IDENTITY(1,1) NOT NULL,user_username varchar(10), user_password varchar(10), user_intentosLogin int, user_status char, user_fechaBaja smalldatetime);
...
CREATE TABLE PASO_A_PASO.Credito (cred_id int IDENTITY(1,1) NOT NULL, cred_cliente int, cred_tipoPago nvarchar(100), cred_fecha smalldatetime,  cred_tarjeta Numeric(10) , cred_monto Numeric(10));
...
CREATE TABLE PASO_A_PASO.Factura(fact_id Numeric(18,0) NOT NULL, fact_desde smalldatetime, fact_hasta smalldatetime, fact_importe int, fact_proveedor int);
