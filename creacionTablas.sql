...
CREATE TABLE Cliente (clie_id int IDENTITY(1,1) NOT NULL, clie_dni Numeric(18,0), clie_nombre nvarchar(255),clie_apellido nvarchar(255), clie_userId int, clie_mail nvarchar(255), clie_telefono Numeric(18,0),clie_direccion nvarchar(255), clie_codigoPostal int, clie_ciudad nvarchar(255),clie_fechaNacimiento smalldatetime);
...
CREATE TABLE Rubro (id_rubro int IDENTITY(1,1) NOT NULL, rubr_nombre nvarchar(100));
...
CREATE TABLE Funcion (func_id int IDENTITY(1,1) NOT NULL, func_nombre varchar(20));
CREATE TABLE FuncionesxRol (id_rol int, id_funcion int);
CREATE TABLE Credito (cred_id int IDENTITY(1,1) NOT NULL, cred_cliente int, cred_tipoPago nvarchar(100), cred_fecha smalldatetime, comp_fecha smalldatetime, cred_tarjeta Numeric(10));
CREATE TABLE Cupon (cupo_id int IDENTITY(1,1) NOT NULL, cupo_fecha smalldatetime, cupo_oferta nvarchar(50), cupo_cliente int);
CREATE TABLE Compra(comp_id int IDENTITY(1,1) NOT NULL, comp_cupon int, comp_fecha smalldatetime, comp_factura Numeric(18,0));
